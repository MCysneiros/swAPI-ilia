...
test.describe('Desafio SWAPI - Planets Requirements', () => {
  ...
  test('Requisito: lista ordenada alfabeticamente', async ({ page }) => {
    ...
    const names = await page.locator('text=/^[A-Z]/ >> visible=true').filter({ 
      has: page.locator('text=Population:') 
    }).allTextContents();
    
    // Remove duplicatas e pega apenas os primeiros
    const planetNames = names.slice(0, 10);
    
    console.log('Planet names:', planetNames);
    
    // Verifica que tem planetas
  ...
  test('Requisito: 10 itens por página com paginação', async ({ page }) => {
    ...
    const paginationExists = await page.getByRole('button', { name: /next|previous|próxima|anterior/i }).count();
  ...
  test('Requisito: cards devem ter nome, terreno, diâmetro e clima', async ({ page }) => {
    await page.waitForSelector('text=Population:', { timeout: 10000 });
