...
export function PlanetDetailContent({ planetId }: PlanetDetailContentProps) {
  ...
  const {
    ...
    error,
    refetch,
  } = usePlanetDetails(planetId);

  const isSyncing = isFetching && !isPlaceholderData && Boolean(planet);

  const films = useMemo(() => planet?.films ?? [], [planet?.films]);
  const residents = useMemo(() => planet?.residents ?? [], [planet?.residents]);
  ...
  return (
    <PageContainer>
      <div className="space-y-6">
        ...
        {films.length > 0 && <PlanetFilms films={films} />}

        {residents.length > 0 && <PlanetResidents residents={residents} />}
